...
document.getElementById("contact-form").addEventListener("submit", function (e) {
  ...
  const serviceID = "service_tw3uy24"; // Replace with your EmailJS Service ID
  const templateToAdmin = "template_o6rtia2"; // Template ID for admin
  const templateToUser = "template_46p5kas"; // Template ID for user
  ...
  // First, notify the admin
  emailjs.sendForm(serviceID, templateToAdmin, form)
    .then(() => {
      console.log("Admin notified");

      // Then, send confirmation to the user
      return emailjs.sendForm(serviceID, templateToUser, form);
    })
    .then(() => {
      console.log("User notified");

      alert("Message sent successfully! You’ll get a confirmation email soon.");
      form.reset();
    })
    .catch((error) => {
    ...
    .finally(() => {
      ...
      submitBtnText.innerText = "Appointment Now";
    });
});